import os, sys, json, uuid, random, asyncio, hmac, base64
...
def _msg_enc_key() -> bytes:
    """
    AES-256 key used to encrypt message content at rest in SQLite.
    Stored in .synora_msg_key (hex). Generated once, persists across restarts.
    Set MSG_ENC_KEY env var (64-char hex) to override.
    """
...
def is_encrypted(stored: str) -> bool:
    """Detect whether stored content looks like our base64 ciphertext."""
    try:
        raw = base64.b64decode(stored)
        return len(raw) >= 29
    except Exception:
...
def _gen_number_sync() -> str:
    """Generate a unique Synora number. Runs in threadpool from async context."""
    with Database() as d:
...
async def gen_number() -> str:
    """Async wrapper — runs DB work in threadpool to avoid blocking the event loop."""
...
def _load_embed_sync():
    """Runs in a thread — downloads ~23 MB model, does not block the event loop."""
    global embed_model
    if not HAS_EMBED:
...
def load_embed():
    """Fire-and-forget: load model in background thread so startup is instant."""
...
def _send_push_to_subscription(subscription_json: str, title: str, body: str, data: dict) -> bool:
    """
    Send a Web Push notification to one subscription (blocking — call from threadpool).
...
async def send_push_notifications(number: str, title: str, body: str, data: dict = None):
    """
    Fetch all stored Web Push subscriptions for `number` and fire push
    notifications in a threadpool so we never block the event loop.
    Expired subscriptions (410 Gone) are pruned automatically.
    """
    if not HAS_WEBPUSH or not VAPID_PRIVATE or not VAPID_PUBLIC:
        return
    data = data or {}
...
async def index():
    ...
    return HTMLResponse("<h1>Synora</h1><p>Run setup — templates/index.html not found.</p>", status_code=500)
...
def _find_user(d: Database, target: str):
    """Find user by number with fallback for zero-padded variants."""
...
async def get_user(target: str, number: str = Depends(get_current_user)):
    ...
    if not user:
        raise HTTPException(404, "No Synora user found with that number. Ask them to share their Synora number with you.")
    return dict(user)
...
async def add_contact(req: Request, number: str = Depends(get_current_user)):
    ...
    if not saved_name or len(saved_name) < 1:
    ...
    with Database() as d:
        ...
        if not user:
            raise HTTPException(
                404,
                f"No Synora user found with number {target}. "
                "Make sure you have the correct number — ask them to share it from their profile."
            )
...
def _start_tunnel(port: int):
    import subprocess, re, platform, stat, urllib.request
    ...
    if system == "linux":
        ...
        print(f"ℹ️  Windows: run `cloudflared tunnel --url http://localhost:{port}` manually")
        return

    if not os.path.exists(binary):
        print("⬇️  Downloading cloudflared (one-time ~30 MB)…")
        try:
    ...
    print("🌐 Opening public tunnel", end="", flush=True)
...
async def export_my_data(number: str = Depends(get_current_user)):
    ...
    Includes: profile, contacts, messages (decrypted), call history.
...
async def delete_my_account(req: Request, number: str = Depends(get_current_user)):
    ...
    Deletes: user row, contacts, messages, call logs.
...
async def list_backup_files(
    ...
    """Admin: list available backup files. Requires SYNORA_ADMIN_KEY env variable."""
...
async def trigger_backup(
    ...
    """Admin: manually trigger a backup right now."""
    expected = os.environ.get("SYNORA_ADMIN_KEY", "")
    if not expected or not hmac.compare_digest(admin_key, expected):
        raise HTTPException(403, "Invalid admin key")
...
async def check_integrity(
    ...
    """Admin: run database integrity check."""
    expected = os.environ.get("SYNORA_ADMIN_KEY", "")
    if not expected or not hmac.compare_digest(admin_key, expected):
        raise HTTPException(403, "Invalid admin key")
...
async def get_audit_log(
    ...
    """Admin: view recent security audit events."""
    expected = os.environ.get("SYNORA_ADMIN_KEY", "")
    if not expected or not hmac.compare_digest(admin_key, expected):
        raise HTTPException(403, "Invalid admin key")
    with Database() as d:
        rows = d.fetchall(
            "SELECT * FROM user_audit ORDER BY ts DESC LIMIT ?", (limit,)
        )
...
def _send_otp_email(email: str, code: str):
    """
    Send OTP via SMTP if configured, otherwise console-log it.
...
async def otp_send(req: Request):
    """Send a 6-digit OTP to an email address (pluggable SMTP)."""
    client_ip = req.client.host if req.client else "unknown"
...
async def otp_verify(req: Request):
    """Verify an OTP. Returns a short-lived verified token."""
    try:
        data = await req.json()
    except Exception:
        raise HTTPException(400, "Invalid request body")
...
async def get_key_backup(number: str = Depends(get_current_user)):
    """
    Return the server-encrypted copy of the user's private key (if stored).
    The private key itself is encrypted client-side with the user's password
    before being sent here — the server never sees the plaintext key.
    """
    with Database() as d:
...
async def put_key_backup(req: Request, number: str = Depends(get_current_user)):
    """
    Store a password-encrypted backup of the private key on the server.
    The payload must be the PKCS8 private key encrypted with AES-256-GCM
    using a key derived from the user's password (PBKDF2, 200k iterations).
    Synora never receives the plaintext key.
    """
...
async def delete_key_backup(number: str = Depends(get_current_user)):
    """Remove the server-side encrypted key backup."""
    with Database() as d:
...
async def submit_report(req: Request, number: str = Depends(get_current_user)):
    """Submit an abuse report against a user or message."""
    ...
    with Database() as d:
        d.execute(
            """INSERT INTO abuse_reports
               (reporter, reported, reason, details, msg_id)
               VALUES (?,?,?,?,?)""",
...
async def push_subscribe(req: Request, number: str = Depends(get_current_user)):
    """Store a Web Push subscription object for this user."""
    try:
        data = await req.json()
    except Exception:
        raise HTTPException(400, "Invalid body")
